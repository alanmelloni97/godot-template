...
var sum_func := func avg (accum: float, number: float) -> float:
	return accum + number
...
func update_settings_label() -> void:
	...
	if viewport.get_camera_3d():
		...
		if viewport.msaa_3d >= Viewport.MSAA_2X:
			antialiasing_3d_string += (" + " if not antialiasing_3d_string.is_empty() else "") + "%d× MSAA" % pow(
				2,
				viewport.msaa_3d,
			)
...
func update_information_label() -> void:
	...
	if RenderingServer.get_video_adapter_vendor().trim_suffix(" Corporation") in RenderingServer.get_video_adapter_name():
		...
		adapter_string = RenderingServer.get_video_adapter_vendor() + " - " + RenderingServer \
				.get_video_adapter_name() \
				.trim_suffix("/PCIe/SSE2")
...
func _fps_graph_draw() -> void:
	...
	fps_graph.draw_polyline(
		fps_polyline,
		frame_time_gradient.sample(remap(frames_per_second, GRAPH_MIN_FPS, GRAPH_MAX_FPS, 0.0, 1.0)),
		1.0,
	)
